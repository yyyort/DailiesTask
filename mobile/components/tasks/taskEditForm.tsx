...
import { TaskCreateSchema, TaskCreateType, TaskReturnType, TaskUpdateSchema, TaskUpdateType } from "@/model/task.model";
...
export default function TaskEditForm({
  ...
  const {
    ...
    defaultValues: {
      ...
      timeToDo: new Date(`${task.deadline}T${task.timeToDo}`).toString(),
      deadline: task.deadline,
      status: task.status,
    },
  });

  const onSubmit: SubmitHandler<TaskUpdateType> = async (
    data: TaskUpdateType
  ) => {
    try {

      const resTask = await updatedTask(task.id,  data);
      ...
      Toast.show({
        type: "success",
        text1: "Success",
        text2: `successfully added task ${resTask.title}`,
          ...
          <View style={styles.textField}>
            ...
            <TouchableOpacity onPress={() => setTimeModal(true)}>
              <ThemedText style={[styles.dateTimeInput]}>
                {
                  new Date(value ?? '').toLocaleTimeString().slice(0, 5) // hh:mm
                }{" "}
                {
